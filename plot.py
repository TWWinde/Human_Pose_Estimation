...
def visualize_short_offsets(offsets, keypoint_id, centers=None, heatmaps=None, radius=config.KP_RADIUS, img=None, every=1, save_path='./'):
    ...
    if isinstance(keypoint_id, str):
        if keypoint_id not in config.KEYPOINTS:
    ...
    plt.savefig(save_path+'short_offsets.jpg', bbox_inches='tight')
    plt.close()
...
def plot_instance_masks(masks, img, save_path='./'):
    ...
    if torch.is_tensor(img):
        img = img.cpu().numpy()
    masks = [mask.cpu().numpy() if torch.is_tensor(mask) else mask for mask in masks]
...
def plot_poses(img, skeletons, save_path=None):
    ...
    if torch.is_tensor(img):
        img = img.cpu().numpy()
    skeletons = [skel.cpu().numpy() if torch.is_tensor(skel) else skel for skel in skeletons]
